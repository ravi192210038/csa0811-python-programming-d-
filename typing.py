...
class MainWindow:
    ...
    def __init__(self, root):
        ...
        self.label_score = Label(
            root, text="Score: ", wraplength=500)
        self.label_score.grid(row=0, column=0, columnspan=3, sticky="nsew")

        self.label_text = Label(
            root, text="Welcome to typing speed calculator", wraplength=500)
        self.label_text.grid(row=1, column=0, columnspan=3, sticky="nsew")

        self.user_text = Text(root)
        self.user_text.grid(column=0, row=2, columnspan=3, sticky="nsew")

        self.btn_start = Button(
            root, text="Start/Restart", command=self.start)
        self.btn_start.grid(column=0, row=3, columnspan=1, sticky="nsew")
        self.btn_newtext = Button(
            root, text="New Text", command=self.new_text)
        self.btn_newtext.grid(column=2, row=3, columnspan=1, sticky="nsew")

        self.label_speed = Label(
            root, text=f"Your typing speed is {self.speed} LPS")
        ...
        # Bind the <KeyRelease> event to the stop method
        self.user_text.bind('<KeyRelease>', self.stop)
    ...
    def stop(self, event=None):
        ...
        chars_typed = len(self.user_text.get("1.0", 'end-1c'))
        self.speed = round(chars_typed / (self.time_end - self.time_start), 2)
        self.label_speed.config(
            text=f"Your typing Speed is {self.speed} LPS")
    ...
    def new_text(self):
        ...
        self.label_score.config(text="Score: ")  # Clear the previous score
